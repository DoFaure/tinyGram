...
public class UnlikeQuery extends HttpServlet{

	@Override
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();	
		Key postToUnlikeKey = KeyFactory.stringToKey(request.getParameter("key"));
		...
		try {
			...
			try {			
				...
				Entity post = datastore.get(postToUnlikeKey);
				ArrayList<String> likes = (ArrayList<String>) post.getProperty("likes");
				likes.remove(user.getKey().getName());
